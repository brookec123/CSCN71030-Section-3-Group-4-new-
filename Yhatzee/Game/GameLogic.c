...
bool isGameDone(GAME_MANAGER g)
{
    ...
    {
        if (!IsGameDoneForPlayer(g.players[p]))
        {
            return false;
        }
    }
...
void whoWon(GAME_MANAGER g)
{
    if (isGameDone(g))
    {
        int currentLargestGrandTotal = g.players[0].currentScore[18][g.players[0].currentGameNumber - 1];
        for (int i = 0; i < g.numOfPlayers; i++)
        {
            if (g.players[i].currentScore[18][g.players[i].currentGameNumber - 1] > currentLargestGrandTotal)
            {
                currentLargestGrandTotal = g.players[i].currentScore[18][g.players[i].currentGameNumber - 1];
            }
        }
        for (int i = 0; i < g.numOfPlayers; i++)
        {
            g.players[i].hasWonCurrentGame = (g.players[i].currentScore[18][g.players[i].currentGameNumber - 1] == currentLargestGrandTotal);
        }
    }
}
